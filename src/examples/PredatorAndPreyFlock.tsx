...
const numberOfPrey = 250

const predatorData = Array.from({ length: numberOfPredators }, () => ({
...
const preyData = Array.from({ length: numberOfPrey }, (_, i) => ({
  position: new THREE.Vector3(random(0, 10), random(0, 10), random(0, 10)),
  acceleration: new THREE.Vector3(0, 0, 0),
  velocity: new THREE.Vector3(0, 0, 0),
  maxSpeed: 0.3,
...
const PredatorAndPreyFlock = () => {
  const [predatorRef, predators] = useVehicles(predatorData)
  const [preyRef, vehicles] = useVehicles(preyData)
  ...
  return (
    <>
      <instancedMesh
        ref={preyRef}
        args={[undefined, undefined, numberOfPrey]}
        castShadow
        receiveShadow
      >
      ...
      <instancedMesh
        ref={predatorRef}
        args={[undefined, undefined, numberOfPredators]}
        castShadow
        receiveShadow
      >
        <cylinderBufferGeometry args={[0, 0.5, 2, 3]} />
...
export default PredatorAndPreyFlock
